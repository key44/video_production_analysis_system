...
皆さまからのフィードバックや活用事例をお待ちしております！本プロジェクトは無料でローカル完結型のツールであり続けながら、コミュニティと共に成長していくことを目指しています。

### 活用事例のシェアのお願い
このシステムを使って、どのような業務改善ができたか教えていただけませんか？
ぜひ [GitHub Discussions](https://github.com/key44/video_production_analysis_system/discussions) の **「Show and tell」** カテゴリに投稿をお願いします。皆さまの実践例が、他のユーザーにとっての大きなヒントになります！

### 機能要望・バグ報告について
バグの報告や、「こんな機能が欲しい」というご要望がありましたら、[GitHub Issues](https://github.com/key44/video_production_analysis_system/issues) からお気軽にご投稿ください。バグ報告時はテンプレートをご利用いただき、新機能の要望には `enhancement` ラベルをご活用ください。

### 利用調査アンケートのお願い
今後の機能改善の参考とするため、簡単なアンケートにご協力をお願いいたします：
👉 **[利用調査アンケート (Microsoft Forms)](https://forms.office.com/r/1NwG84MH9m)**

- **個人情報は一切不要**：会社名や氏名など、個人情報を入力する項目はありません。完全に**任意・匿名**でのご回答となります。
- **シンプルな構成**：「本システムの利用用途や良い点、悪い点、改善点があればご記入下さい。」という自由記入欄が1つあるだけの簡単なフォームです。
- **なぜアンケートが必要か**：このシステムは、ソフトウェア自体が外部へ情報を自動送信しないセキュアな設計になっています。そのため、開発者が「ツールがどのように使われているか」を知る**唯一の手段**がこのアンケートになります。ぜひご協力をお願いします。

## プライバシーとセキュリティ

**外部への情報送信は一切ありません。**
本アプリケーションはすべてお使いのブラウザ内（ローカル環境）で完結して動作します。テレメトリや自動データ収集の仕組み等は一切組み込まれておらず、皆さまの動画データや分析データが意図せず外部に送信されることは絶対にありません。

## ライセンス

本プロジェクトは **GPL-3.0** ライセンスのもとで公開されています。

このソフトウェアは自由に使用、改変、再配布することができます。ただし、改変したソフトウェアを再配布する場合は、同じGPL-3.0ライセンスのもとでソースコードを公開する必要があります。詳細な利用条件については、リポジトリ内の [LICENSE](LICENSE) ファイルをご参照ください。
